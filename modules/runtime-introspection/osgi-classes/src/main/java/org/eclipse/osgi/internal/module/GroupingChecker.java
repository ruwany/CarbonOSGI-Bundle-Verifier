...
import org.eclipse.osgi.internal.wso2.LoadingIntrospection;
                ...
                for (int j = 0; j < uses.length; j++) {
                    ...
                    if (thisUsedRoots != nullPackageRoots && importingUsedRoots != nullPackageRoots)
                        if (!(subSet(thisUsedRoots.roots, importingUsedRoots.roots) || subSet(importingUsedRoots.roots, thisUsedRoots.roots))) {
                            ...
                            if(LoadingIntrospection.isEnabled()) {
                                ExportPackageDescription rootBase = ((ExportPackageDescription) ((ResolverExport) thisUsedRoots
                                        .getRoots()[0]).base);
                                String rootExporterName = rootBase.getExporter().getLocation();
                                String rootImportName = rootBase.getName() + ":" + rootBase.getVersion();

                                ExportPackageDescription importingBase = ((ExportPackageDescription) ((ResolverExport) importingUsedRoots
                                        .getRoots()[0]).base);
                                String importingExporterName = importingBase.getExporter().getLocation();
                                String importingImportName = importingUsedRoots.getName() + ":" + rootBase.getVersion();
                                ;

                                System.out.println(
                                        "[ERROR] Loading Class ["+LoadingIntrospection.getIntroClassName()+ "] failed. There is a conflict in package: " + rootExporterName + " Exporting " + rootImportName + "\n"
                                                + importingExporterName + " Exporting " + importingImportName);
                            }
                        }
